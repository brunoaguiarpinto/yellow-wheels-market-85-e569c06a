...
const DailyTasksPanel = () => {
  ...
  useEffect(() => {
    ...
    const todayTasks = savedTasks.filter((task: Task) => task.dueDate === today && task.employeeId === user?.id && task.status !== TaskStatus.COMPLETED);

    // Filtrar compromissos do dia atual para o usuário logado
    const todayAppointments = savedAppointments.filter((appointment: Appointment) => appointment.date === today && appointment.employeeId === user?.id && appointment.status !== 'completed');
    setTasks(todayTasks);
  ...
  const completeTask = (taskId: string) => {
    ...
    const updatedTasks = allTasks.map((task: Task) => task.id === taskId ? {
      ...task,
  ...
  return <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {tasks.length === 0 ? <p className="text-gray-500 text-center py-4">
                Nenhuma tarefa pendente para hoje! 🎉
              </p> : tasks.map(task => <div key={task.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex-1">
                    ...
                    {task.customerName && <p className="text-sm text-blue-600">Cliente: {task.customerName}</p>}
                    {task.dueTime && <p className="text-sm text-gray-500">Horário: {task.dueTime}</p>}
                  </div>
                  <Button size="sm" onClick={() => completeTask(task.id)} className="ml-4">
                    Concluir
                  </Button>
                </div>)}
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {appointments.length === 0 ? <p className="text-gray-500 text-center py-4">
                Nenhum compromisso agendado para hoje.
              </p> : appointments.map(appointment => <div key={appointment.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex-1">
